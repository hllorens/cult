        ...
        foreach(array_keys($stock_financial) as $stock_financial_period){
            ...
            if(substr($period_arr[1][$period],0,4)==substr($stock_financial_period,0,4)){
                ...
                if($new_month>$original_month){
                    $new_period=true;
                    echo "<br />ERROR: assets period change same year new ".$period_arr[1][$period]." vs existing ".$stock_financial_period." keeping the one with higher month<br />";
                    send_mail('Error assets '.$name,"<br />assets period change same year new ".$period_arr[1][$period]." vs existing ".$stock_financial_period.", keeping the one with higher month <br /><br />","hectorlm1983@gmail.com");
        ...
        foreach($vars2get as $var2get){
            ...
            if(!array_key_exists($var2get,$stock_financial[$period_arr[1][$period]])){
                if($results[$var2get][$period]=="-" || $results[$var2get][$period]==""){
                    ...
                    send_mail('Error assets '.$name,"<br />Empty - in $var2get (stock_curl_financial.php), setting 0<br /><br />","hectorlm1983@gmail.com");
                }
